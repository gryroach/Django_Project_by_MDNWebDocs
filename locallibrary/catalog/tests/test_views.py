...
class AuthorCreateTest(TestCase):
    ...
    def setUp(self):
        ...
        # Создание автора
        self.test_author1 = Author.objects.create(first_name='John', last_name='Smith')

    def test_redirect_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='testuser1', password='12345')
        author_create = {'first_name': 'Sam', 'last_name': 'Black'}
        resp = self.client.post(reverse('author_update', kwargs={'pk': self.test_author1.pk, }),
                                author_create)
        self.assertEqual(resp.status_code, 302)

    def test_logged_in_with_permission_update_author(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.get(reverse('author_update', kwargs={'pk': self.test_author1.pk, }))
